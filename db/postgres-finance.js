...
async function getUserFinancialSummary(userId) {
  ...
  const [banks, expYearlyR, expMonthlyR, expCategoryR, expDailyR, recentExpensesR, friendsR, emisR, cardsR, tripsR, plannerPaymentsR, defaultsR] = await Promise.all([
    pgOpsDb.getBankAccounts(userId),
    ...
    query(`SELECT purchase_date::text AS day, COALESCE(SUM(amount),0) AS total, COUNT(*)::int AS count FROM expenses WHERE user_id = $1 AND deleted_at IS NULL AND purchase_date >= (CURRENT_DATE - INTERVAL '365 days') GROUP BY purchase_date ORDER BY purchase_date DESC`, [userId]),
  ...
  return {
    ...
    expense_by_day: expDailyR.rows.map((row) => ({ ...row, total: num(row.total), count: Number(row.count) })),
